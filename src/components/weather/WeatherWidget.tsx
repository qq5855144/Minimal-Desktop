...
export default function WeatherWidget({ item, preview = false }: { item?: DesktopItem; preview?: boolean }) {
  ...
  const { city, data, loading, error } = weather;
  ...
  return <>
    <button type="button" data-press-intent-surface="true"
      ...
      aria-label={city ? `${city.name}天气，${degrees(data?.temp)}，查看详情` : '设置天气城市'}
      onClick={() => !preview && setOpen(true)}>
      <div className="weather-card-main">
        <div className="weather-city"><MapPin size={14} /><span>{city?.name ?? '选择城市'}</span></div>
        ...
        <div className="weather-card-hint"><Umbrella size={14} /><span>{error ? '更新失败 · 查看详情' : !city ? '点击设置城市' : !data ? '点击查看天气' : today?.rain == null ? '查看天气趋势' : `今日降水概率 ${Math.round(today.rain)}%`}</span></div>
      </div>
